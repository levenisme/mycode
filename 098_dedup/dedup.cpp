...
void dir_recursor(const std::string & dir, std::unordered_map<size_t, std::string> & myhashmap) {
  ...
  if ((curr_dir = opendir(dir.c_str())) ==
      0) {  // if I new a string myself and c_str it, it returns a pointer which could cause dangling pointer problem. be careful!
